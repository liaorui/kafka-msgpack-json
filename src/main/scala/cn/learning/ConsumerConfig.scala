...
object ConsoleConsumer extends Logging {
  ...
  var messageCount = 0
  var protoDesc: String = null

  private val shutdownLatch = new CountDownLatch(1)

  def main(args: Array[String]) {
    val conf = new ConsumerConfig(args)
    if (conf.options.has(conf.protoOpt)) {
      // check proto command
      var result = "hash protoc".!
      if (result == 1) {
        System.err.println("protoc require but it's not installed. Aborting.")
        System.exit(1)
      }

      val proto = conf.options.valueOf(conf.protoOpt)
      result = ("protoc --include_imports --descriptor_set_out=" + proto + ".desc" + " " + proto).!
      if (result == 1) {
        System.err.println("generate proto descriptor file failed. Aborting.")
        System.exit(1)
      }
  ...
  class ConsumerConfig(args: Array[String]) {
    ...
    val protoOpt = parser.accepts("proto", "The protobuf message format file.")
      .withRequiredArg
      .describedAs("proto")
...
class DefaultMessageFormatter extends MessageFormatter {
  ...
  def writeTo(consumerRecord: ConsumerRecord[Array[Byte], Array[Byte]], output: PrintStream) {
    ...
    def write(deserializer: Option[Deserializer[_]], sourceBytes: Array[Byte], separator: Array[Byte]) {
      ...
      var parser: Parser = new MsgPackParser
      if (ConsoleConsumer.protoDesc != null) {
        val protoDescBuf = Files.readAllBytes(Paths.get(ConsoleConsumer.protoDesc))
        parser = new ProtobufParser(protoDescBuf)
      }

      try {
        output.write(parser.parse(convertedBytes).getBytes)
        output.write(separator)
      } catch {
        case e: Throwable =>
          System.err.println("Unknown error when running consumer: ", e)
          System.exit(1)
      }
